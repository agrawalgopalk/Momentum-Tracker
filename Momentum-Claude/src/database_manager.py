"""
database_manager.py – Persistent + in-memory cache for price and
                       fundamental data.
...
class DatabaseManager:
    ...
    def get_price(self, ticker: str) -> Optional[pd.DataFrame]:
        ...
        return self._price_mem.get(ticker) or self._benchmark_mem.get(ticker)
        ...
        if cache_exists:
            try:
                ...
                if df_cached is None or df_cached.empty:
                    ...
                    # Cache is fresh – check history depth
                    cache_start = df_cached.index.min().date()
                    if cache_start > required_start:
                        print(
                            f"[DB] {ticker} cache history ({cache_start}) "
                            f"< {config_years} years → full re-download."
                        )
                        trigger_full = True
                    else:
