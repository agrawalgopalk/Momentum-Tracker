...
(Updated: uses search_tool_setup.py so no SERPER_API_KEY is required)
"""

from crewai import Agent, Task, Crew
from llm_config import LLMConfig
from momentum_tool import MomentumBackboneTool
from search_tool_setup import search_tool          # ← replaces SerperDevTool
...
task_scan = Task(
    description=(
        ...
        "multi-factor WMS pipeline - RSI, MFI, CCI, rate-of-change composites, "
        "and relative strength vs Nifty 50 - then applies price, volume, and "
        "percentile filters.\n\n"
        ...
        "  3. A one-line note on any obvious sector cluster "
...
MOMENTUM QUALITY CHECK - choose one:
  SUPPORTED   - momentum backed by improving earnings / positive sector cycle / institutional buy
  STRETCHED   - price has run significantly ahead of fundamentals; caution on fresh entry
  UNSUPPORTED - purely technical move; fundamentals deteriorating or neutral; avoid

RISK FLAGS (up to 3, or "None"):
  e.g. High promoter pledge | Regulatory overhang | Leverage spike | Sector rotation risk

ONE-LINE RATIONALE:
  Single sentence summarising the investment case or why you are cautious.
---
...
After all stock blocks write a summary covering:
