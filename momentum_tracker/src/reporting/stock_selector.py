...
class StockSelector:
    ...
    def top_recommendations(
        ...
        if self._exporter is not None and not export_df.empty:
            output_dir = self._cfg.get("SYSTEM_CONFIG", {}).get("OUTPUT_DIR", "output")
    ...
    def export_all_raw_scores(self, category: str) -> None:
        ...
        """
        from data.symbol_loader import SymbolLoader # Ensure circular imports handled
        ...
        results_dir = Path(self._cfg.get("SYSTEM_CONFIG", {}).get("BACKTEST_RESULTS_DIR", "Backtest_Results"))
        results_dir.mkdir(parents=True, exist_ok=True)
        out_path = results_dir / f"Full_Universe_{category}_{ts}.xlsx"
    ...
    def score_custom_universe_from_excel(self, file_name: str) -> None:
        ...
        if all_results:
            ts       = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_dir = Path(self._cfg.get("SYSTEM_CONFIG", {}).get("OUTPUT_DIR", "output"))
            out_file = output_dir / f"custom_universe_scores_{ts}.xlsx"
