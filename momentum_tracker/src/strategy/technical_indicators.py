...
class TechnicalIndicators:
    ...
    # ─────────────────────────────────────────────────────────────────────
    # Series-based equivalents for historical calculations
    # ─────────────────────────────────────────────────────────────────────

    @staticmethod
    def ema_series(df: pd.DataFrame, period: int = 50) -> pd.Series:
    ...
    def rsi_series(df: pd.DataFrame, period: int = 14) -> pd.Series:
        """Wilder RSI (Relative Strength Index) Series."""
        if df.empty or "close" not in df.columns or len(df) < period:
            return pd.Series(np.nan, index=df.index)
        try:
            delta = df["close"].diff()
    ...
    @staticmethod
    def roc_series(df: pd.DataFrame, period: int = 20) -> pd.Series:
        """Rate of Change (percentage) Series."""
        if df.empty or "close" not in df.columns or len(df) <= period:
            return pd.Series(np.nan, index=df.index)
        try:
    ...
    @staticmethod
    def cci_series(df: pd.DataFrame, period: int = 20) -> pd.Series:
    ...
    @staticmethod
    def weighted_roc_composite_series(
        df: pd.DataFrame,
        periods: list[int],
        weights: list[float],
    ) -> pd.Series:
        """Composite ROC score series."""
        if df.empty or "close" not in df.columns or len(periods) != len(weights):
            return pd.Series(np.nan, index=df.index)
        try:
            total_w = sum(weights)
            ...
            for p, w in zip(periods, weights):
                ...
                weighted_sum += roc.fillna(0.0) * w
            return (weighted_sum / total_w) * 100
        except Exception:
            return pd.Series(np.nan, index=df.index)
    ...
    def rs_ratio_series(
        ...
        """Calculates the RS Ratio Series (Return-based Relative Strength)."""
        if stock_df is None or bench_df is None or stock_df.empty or bench_df.empty:
            return pd.Series(np.nan, index=stock_df.index if stock_df is not None else [])
        try:
            combined = pd.concat([stock_df['close'], bench_df['close']], axis=1, join='inner')
            combined.columns = ['stock', 'bench']
            stock_return = combined['stock'] / combined['stock'].shift(lookback)
            bench_return = combined['bench'] / combined['bench'].shift(lookback)
            res = (stock_return / bench_return) - 1.0
            return res.reindex(stock_df.index)
        except Exception:
            return pd.Series(np.nan, index=stock_df.index)

    @staticmethod
    def price_momentum_composite_series(df: pd.DataFrame) -> pd.Series:
        """Multi-period price momentum composite series."""
        if df.empty or "close" not in df.columns:
            return pd.Series(np.nan, index=df.index)
        try:
            close = df["close"]
            roc_12m = close.pct_change(252)
            roc_6m  = close.pct_change(126)
            roc_3m  = close.pct_change(63)
            high_52w = df["high"].rolling(252).max() if "high" in df.columns else pd.Series(np.nan, index=df.index)
            dist_from_high = (close / high_52w) - 1.0

            # Weights: 12m (1.0), 6m (2.0), 3m (2.0), dist (0.5)
            score = (roc_12m.fillna(0.0) * 1.0 +
                     roc_6m.fillna(0.0) * 2.0 +
                     roc_3m.fillna(0.0) * 2.0 +
                     dist_from_high.fillna(0.0) * 0.5) / 5.5
            return score * 100
        except Exception:
            return pd.Series(np.nan, index=df.index)
