...
class SymbolLoader:
    ...
    def add_category(self, category_name: str, file_name: str, benchmark_ticker: Optional[str] = None) -> bool:
        """
        Programmatic helper to add a new category to DATA_CONFIG.SYMBOL_FILE_MAP.
        This updates the in-memory config and persists it via Config.save().
        """
        file_map = self._cfg["DATA_CONFIG"].get("SYMBOL_FILE_MAP", {})
        if not isinstance(file_map, dict):
        ...
        if not category_name or not file_name:
            return False
        ...
        file_map[category_name] = file_name
        if benchmark_ticker:
            idx_map[category_name] = benchmark_ticker
