...
# streamlite_app/dashboard.py  — top of file, before all imports
import sys
...
# streamlite_app/dashboard.py → parent = streamlite_app/ → parent = Momentum-Tracker/
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    log_path = get_log_file().parent / ("scheduler.log" if "scheduler" in log_choice else "momentum_dashboard.log")
    if log_path.exists():
        lines = log_path.read_text(encoding="utf-8").splitlines()
        n_lines = st.slider("Last N lines", 20, 200, 50)
        st.code("\n".join(lines[-n_lines:]), language=None)
    else:
        st.info(f"{log_path.name} not found yet.")        
